 ...
 * Isolated counter: project decisiongate only.
 */

const PROJECT = "decisiongate";
const DEFAULT_ASSET = "decisiongate-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "decisiongate";
const DEFAULT_BRANCH = "main";
const GITHUB_RELEASES = "https://github.com/AzielEliab/decisiongate/releases";
const GITHUB_LATEST = "https://github.com/AzielEliab/decisiongate/releases/latest";
const HOST = "https://decisiongate-download-tracker.vibelock.workers.dev";
const GITHUB_REPO = "https://github.com/AzielEliab/decisiongate";
const INSTALL_LINE = "curl -fsSL https://decisiongate-download-tracker.vibelock.workers.dev/install.sh | bash";
const DOI = "https://doi.org/10.5281/zenodo.21435730";
const ZENODO = "https://zenodo.org/records/21435730";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 DecisionGATE-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/decisiongate", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/decisiongate/releases/latest", { headers });
    if (relRes.ok) {
...
# DecisionGATE one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
...
<script type="application/ld+json">
{
  ...
  "codeRepository": "https://github.com/AzielEliab/decisiongate",
...
<body>
  ...
  <div id="meshStrip" aria-label="Suite Live Nodes">
    ...
    <div>
      ...
      <button id="meshJoin" type="button" title="Join as decisiongate. Refused while mesh is OFF. No auto-join.">Join</button>
    ...
    <script>
      (function () {
        ...
        btn.addEventListener("click", function () {
          function done(ok) {
            btn.textContent = ok ? "Copied! Paste in Terminal, then run decisiongate ui" : "Select the command, copy it, then run decisiongate ui";
      ...
      (function () {
        ...
        function paintMesh(raw) {
          ...
          $("meshProducts").textContent = "Catalog MCP mesh_* · FragGate slug=mesh · /v1/mesh/* PROXY · QNS-CD-1.0 hub cite · not AnonBroadcast · not AZMail ring · not a Node Gate · no public qnsd proxy" + extra;
        }
        ...
        async function refreshMesh() {
          try {
            ...
            paintMesh(merged);
            var nodeId = sessionStorage.getItem("decisiongate_mesh_node");
            if (on && nodeId) {
        ...
        $("meshDisable").onclick = async function () {
          sessionStorage.removeItem("decisiongate_mesh_node");
          paintMesh(await meshPost("/v1/mesh/disable", {}));
          refreshMesh();
        };
        $("meshJoin").onclick = async function () {
          var j = await meshPost("/v1/mesh/join", { product: "decisiongate", label: "DecisionGATE Worker" });
          var inner = unwrapMesh(j);
          var id = inner.node_id || inner.id || (inner.session && inner.session.node_id);
          if (id) sessionStorage.setItem("decisiongate_mesh_node", String(id));
          paintMesh(j);
          refreshMesh();
        };
        $("meshLeave").onclick = async function () {
          var id = sessionStorage.getItem("decisiongate_mesh_node");
...
export default {
  async fetch(request, env) {
    ...
    if ((url.pathname === "/cite.json" || url.pathname === "/cite.json/") && request.method === "GET") {
      return json({"author": "Aziel Eliab", "title": "DecisionGATE", "github": "https://github.com/AzielEliab/decisiongate", "download": "https://decisiongate-download-tracker.vibelock.workers.dev/download", "doi": "10.5281/zenodo.21435730", "license": "Apache-2.0", "catalog": "https://aziel-runtime.vibelock.workers.dev/"});
    }
    // /gitbaby-seo-routes
