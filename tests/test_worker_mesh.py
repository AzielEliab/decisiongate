...
Default OFF. live|locked|isolated. QNS-CD hub cite only. No Node Gate.
No public qnsd proxy. No auto-heal. Not anonymity.
"""
...
def test_mesh_contract_default_off_qnm_law() -> None:
    ...
    assert 'MESH_PRODUCT = "decisiongate"' in MESH
...
def test_mesh_pointer_and_openapi_helpers() -> None:
    ...
    assert "decisiongate_mesh_" in MESH
...
def test_mesh_proxies_via_aziel_runtime() -> None:
    ...
    assert 'id = "147130a914ed4fac8632570871d2c266"' in WRANGLER
...
def test_home_live_nodes_strip_no_node_gate() -> None:
    ...
    assert 'product: "decisiongate"' in INDEX
